...
struct ShortcutConfig {
    ...
    var baseString: String {
        let parts = modifierNames(from: baseModifiers) + (baseKey != 0 ? [keyCodeToString(baseKey)] : [])
        return parts.isEmpty ? "None" : parts.joined(separator: "+")
    }
    
    var forwardShortcutString: String { shortcutString(forwardModifiers, forwardKey) }
    var backwardShortcutString: String { shortcutString(backwardModifiers, backwardKey) }
...
class FloatingTitleView: NSView {
    ...
    private let titleLabel = makeLabel(size: 13, color: .white, align: .center)
    ...
    private func setupViews() {
        ...
        backgroundView.layer?.backgroundColor = NSColor.systemBlue.withAlphaComponent(0.3).cgColor
...
class WindowItemView: BaseItemView {
    ...
    private let appNameLabel = makeLabel(size: 12, weight: .semibold, color: .secondaryLabelColor)
    private let appNameContainerView = NSView()
    ...
    private static func createBadge(color: NSColor, symbol: String, fontSize: CGFloat, yOffset: CGFloat) -> NSView {
        ...
        b.isHidden = true
        let l = makeLabel(symbol, size: fontSize, weight: .bold, color: .white, align: .center)
        b.addSubview(l)
        b.size(20, 20)
        l.center(in: b)
    ...
    private func setupViews() {
        ...
        add(appNameContainerView, thumbnailContainer, iconImageView, titleContainerView, minimizedLabel, floatingAppIconView, minimizedBadge, fullscreenBadge)
        appNameContainerView.add(smallAppIconView, appNameLabel)
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        if isSelected {
            ...
            layer?.backgroundColor = NSColor.white.withAlphaComponent(0).cgColor
        } else {
...
class ListItemView: BaseItemView {
    private let iconImageView = NSImageView()
    private let titleLabel = makeLabel(size: 13, color: .white)
    ...
    override func draw(_ dirtyRect: NSRect) {
        super.draw(dirtyRect)
        layer?.backgroundColor = isSelected ? NSColor.systemBlue.withAlphaComponent(0.3).cgColor : (isActive ? NSColor.white.withAlphaComponent(0).cgColor : NSColor.clear.cgColor)
    }
}
...
class SwitcherWindowController {
    ...
    private func scheduleHide() {
        hideTimer?.invalidate()
        hideTimer = Timer.scheduledTimer(withTimeInterval: Config.autoHideDelay, repeats: false) { [weak self] _ in self?.hide() }
    }
